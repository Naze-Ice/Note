...
Java内存模型是为了**屏蔽各种硬件和操作系统的内存访问差异**，以实现Java在各平台下都能达到一致的内存访问效果

#### 主内存与工作内存

处理器上寄存器的读写速度比内存快几个数量级，为屏蔽这种差距加入了高速缓存

加入高速缓存带来了缓存一致性问题，通过加入缓存一致性性协议来解决

![](images/68747470733a2f2f63732d6e6f7465732d313235363130393739362e636f732e61702d6775616e677a686f752e6d7971636c6f75642e636f6d2f39343263613064322d396435632d343561342d383963622d3566643839623631393133662e706e67.png)

所有变量都存储在主内存，每个线程都有自己的工作内存（寄存器或高速缓存中），保存了该线程使用的变量的主内存副本拷贝

线程只能直接操作工作内存，不同线程的变量值传递需要通过主内存完成

![](images/68747470733a2f2f63732d6e6f7465732d313235363130393739362e636f732e61702d6775616e677a686f752e6d7971636c6f75642e636f6d2f31353835313535352d356162632d343937642d616433342d6566656431306634336136622e706e67 (1).png)

#### 内存间交互操作

主内存与工作内存之间的交互协议，虚拟机实现时必须保证以下操作的原子性

![](images/68747470733a2f2f63732d6e6f7465732d313235363130393739362e636f732e61702d6775616e677a686f752e6d7971636c6f75642e636f6d2f38623765626261642d393630342d343337352d383465332d6634313230393964313730632e706e67.png)
...
>  JVM规范允许虚拟机将long和double类型的非volatile数据的读写操作分为两次32位的操作来进行，但是几乎所有平台的商用虚拟机都选择把64位数据的读写操作作为原子操作来对待，否则需要手动添加volatile

2. 可见性

当一个线程修改了共享变量的值，其他线程能立即得知这个修改。JMM通过**在变量修改后将新值同步回主内存，在变量读取前从主内存刷新变量值来实现可见性**

主要有三种实现可见性的方式

- volatile，每次都从主内存读，写之后立刻同步回主内存
- synchronized，对变量执行unlock前必须同步回主内存
- final，被 final 关键字修饰的字段在构造器中一旦初始化完成，并且没有发生 this 逃逸，则其它线程就能看见 final 字段的值

> this逃逸指在构造函数返回之前，其他线程就持有该对象的引用，通常发生在构造函数中启动线程或注册监听器
...
private class EscapeRunnable implements Runnable {      @Override
   public void run() {          
...
在构造函数中创建Thread对象是没有问题的, 但是不要启动Thread. 可以提供一个init方法
...
- synchronized 保证一个变量在每个时刻只允许一个线程执行lock操作，相当于是让线程顺序执行同步代码

有序性不仅仅靠volatile和synchronized来完成，还包括先行发生原则（**happens-before**），让一个操作无需控制就能先于另一个操作完成

- 单一线程规则：单线程中，前面的操作先于后面的操作
- 管程锁定规则：unlock操作先于后面对同一锁的lock操作
- volatile变量规则：volatile变量的写操作先于后面的读操作
- 线程启动规则：Thread 对象的 start() 方法调用先行发生于此线程的每一个动作

> 即线程A在执行线程B的start方法之前修改了共享变量的值，那么当线程B执行start方法时，线程A对共享变量的修改对线程B可见
...
- 对象终结规则：对象的初始化完成（构造函数执行结束）先于它的finalize()
- 传递性：操作A先于B，B先于C，则A先于C
